# Copyright (C) 2019 The Raphielscape Company LLC.; Licensed under the Raphielscape Public License, Version 1.d (the "License"); you may not use this file except in compliance with the License.; Thanks @rupansh
...
@register(outgoing=True, pattern="^\{trg}kang".format(trg=trgg))
async def kang(args):
...
@register(outgoing=True, pattern="^\{trg}ok".format(trg=trgg))
async def oldkang(args):
    """ For .kang command, kangs stickers or creates new ones. """
    user = await bot.get_me()
    if not user.username:
...
@register(outgoing=True, pattern="^\{trg}stkrinfo$".format(trg=trgg))
...
@register(outgoing=True, pattern="^\{trg}getsticker$".format(trg=trgg))
...
@register(outgoing=True, pattern="^\{trg}findsticker (.*)".format(trg=trgg))
...
CMD_HELP.update(
    {
       ...
       "\n>`.ok <emoji>[optional] <pack number>[optional]`"
        "\nUsage: Adds sticker or image to your userbot pack that is generated by og fizilion keeping the old naming silently."
        "\n\n>`.stkrinfo`"
